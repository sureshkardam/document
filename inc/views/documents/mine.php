            ...
            <table id="dataTable" class="responsive-table striped highlight bordered" cellspacing="0">
                                ...
                                <tbody>
                                    <?
                                    if($user->userinfo->id=='8')
									{
										$documents = $db->select('documents',array(),array('created'=>'desc'));
                                    ...
                                    <tr>
                                        ...
                                        <td class='actions'>
                                         <?php if($u->can_print=='0'){ ?>
											  <a style="color:red" href="#" onclick="get_download(<?php echo $u->id?>)" class='tooltipped'  data-position="top" data-delay="10" data-tooltip="Download"><i class="material-icons">file_download</i></a>
										 <?php }else{ ?>
									...
									{
									$documents = $db->select('documents',array('user'=>$user->userinfo->id),array('created'=>'desc'));
									foreach($documents as $u){
                                            $template = $db->select_row('templates',array('id'=>$u->template));
                                            $user = $db->select_row('users',array('id'=>$u->user));
